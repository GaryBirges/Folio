...
//  spec      = o => x => Object.keys(o)
//   .map(k => this.objOf(k)(o[k](x)))
//   .reduce((acc, o) => Object.assign(acc, o))

//  objOf     = k => v => { var o = {}; o[k] = v; return o }
...
//  nextMoves(state){
//   return state.moves.length > 1 ?  state.moved.slice(1): state.moves

//  } 
//  /*dropFirst(state.moves) : state.moves*/

//  nextApple(state) {
//   return this.willEat(state) ? this.rndPos(state) : state.apple
//  } 
//  nextHead  = state => state.snake.length == 0
//   ? { x: 2, y: 2 }
//   : {
//     x: this.mod(state.cols,state.snake[0].x + state.moves[0].x),
//     y: this.mod(state.rows,state.snake[0].y + state.moves[0].y)
//   }
//  nextSnake = state => this.willCrash(state)
//   ? []
//   : (this.willEat(state)
//     ? [this.nextHead(state)].concat(state.snake)                                              
//     : [this.nextHead(state)].concat(state.snake.slice(0,state.snake.length-1 )))          //droplast

// // Randomness
//  rndPos = table => ({
//   x:  Math.floor(Math.random() * (table.cols - 1)) + 0,
//   y:  Math.floor(Math.random() * (table.rows - 1)) + 0,
// })
...
//  enqueue = (state, move) => this.validMove(move,state)
//   ? this.merge(state)({ moves: state.moves.concat([move]) })
//   : state
...
//   // draw snake
//   this.ctx.fillStyle = 'rgb(0,200,50)'
//   this.state.snake.map(p => this.ctx.fillRect(x(p.x), y(p.y), x(1), y(1)))

//   // draw apples
//   this.ctx.fillStyle = 'rgb(255,50,0)'
//   this.ctx.fillRect(x(this.state.apple.x), y(this.state.apple.y), x(1), y(1))
...
window.addEventListener('keydown', e => {
  switch (e.key) {
    case 'w': case 'h': case 'ArrowUp':    this.state = enqueue(this.state, NORTH); break
    case 'a': case 'j': case 'ArrowLeft':  this.state = enqueue(this.state, WEST);  break
    case 's': case 'k': case 'ArrowDown':  this.state = enqueue(this.state, SOUTH); break
    case 'd': case 'l': case 'ArrowRight': this.state = enqueue(this.state, EAST);  break
  }
})
